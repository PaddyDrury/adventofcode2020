...
class Day20(inputFile: String) {
    ...
    private val matchingCombos = tiles.simpleCombinations(2).filter { it.first().allPossibleSides.intersect(it.last().allPossibleSides).isNotEmpty() }
    ...
    fun isMonsterAt(column: Int, row: Int, grid: Grid) = monsterMap.all { grid[row+it.second][column+it.first] == '#' }
        ...
        return assembled.count('#') -
                (assembled.orientations().map { grid ->
                    grid.indices.toList().dropLast(seaMonster.size - 1)
                        .flatMap { row ->
                            grid.first().indices.toList().dropLast(seaMonster.first().length - 1).map { col -> Pair(col, row) }
